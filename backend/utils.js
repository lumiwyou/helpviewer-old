            ...
            if (Object.keys(entry).includes("headers")) {
                if (Array.isArray(entry.headers)) {
                    ...
                    entry.headers.forEach((header) => {
                        if (
                            header != null &&
                            Object.keys(header).includes("#text")
                        ) {
                            if (
                                header["#text"].toString().includes(` ${term} `)
                            )
                                found = true;
                        }
                    });
            ...
            if (Object.keys(entry).includes("codesnippets")) {
                if (Array.isArray(entry.codesnippets)) {
                    ...
                    if (
                        ...
                        if (
                            entry.codesnippets["#text"].toString().include(term)
                        )
                            found = true;
                    }
                }
...
export const generateIndex = function generateIndex() {
    ...
    products.forEach((product, p) => {
        ...
        try {
            var index_entry = {
                filepath: product,
                title: extractTags(root, "title"),
                headers: extractTags(root, "h2"),
                codesnippets: extractTags(root, "codesnippet"),
            };
